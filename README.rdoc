...
While these deployments fit the requirements for many auto-scaling HA
...
The different cookbooks depend on other cookbooks for resources, providers, and
definitions. If a dependency is not specified in the <tt>metadata.rb</tt> for a
cookbook, its recipes may load without the dependent cookbooks and will fail to
...
Another pattern used in the RightScale cookbooks is the Abstract Cookbooks and
Light Weight Resources and Providers pattern. In this pattern the abstract
cookbook contains a resource that defines a set of general actions (for example
initializing, backing up, and restoring a database) and a set of recipes that
use these actions. Then, the "concrete" cookbooks contain providers that
...
== CUSTOMIZATION:
TODO what is the best way to make changes?


== CONTRIBUTING:
TODO how do I contribute changes back?  Is there an agreement I have to sign?
