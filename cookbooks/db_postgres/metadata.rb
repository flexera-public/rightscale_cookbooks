...
recipe  "db_postgres::setup_server_9_1", "Set the DB PostgresSQL provider. Sets version 9.1 and node variables specific to PostgresSQL 9.1."
recipe  "db_postgres::do_set_slave_sync_mode", <<-EOH
Set master to do sync-based replication with slaves. PostgreSQL streaming replication is asynchronous by default. If the primary server crashes then some transactions that were committed may not have been replicated to the standby server, causing data loss. The amount of data loss is proportional to the replication delay at the time of failover.
...
EOH
recipe  "db_postgres::do_set_slave_async_mode", "Set master to do async-based replication with slaves. For more info refer: http://www.postgresql.org/docs/9.1/static/warm-standby.html#SYNCHRONOUS-REPLICATION"
recipe  "db_postgres::do_show_slave_sync_mode", "Show the sync mode used for replication."
