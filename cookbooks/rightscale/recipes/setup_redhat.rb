...
raise "'rightscale/redhat/username' input must be set!" if
  node[:rightscale][:redhat][:username].to_s.empty?
raise "'rightscale/redhat/password' input must be set!" if
  node[:rightscale][:redhat][:password].to_s.empty?

# 'rhnreg_ks' is a utility for registering a system with the
# RHN Satellite or Red Hat Network Classic.
cmd = "rhnreg_ks --username=#{node[:rightscale][:redhat][:username]}"
cmd << " --password=#{node[:rightscale][:redhat][:password]} --use-eus-channel"
