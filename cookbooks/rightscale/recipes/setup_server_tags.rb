...
while node[:cloud][:private_ips] &&
  RightScale::Utils::Helper.is_valid_ip?(node[:cloud][:private_ips][i]) do
...
while node[:cloud][:public_ips] &&
  RightScale::Utils::Helper.is_valid_ip?(node[:cloud][:public_ips][i]) do
