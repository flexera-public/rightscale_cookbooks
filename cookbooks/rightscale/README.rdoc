...
the RightScale-Epel repository.  Runs if the <tt>rightscale/security_update</tt>
input is set to "Enabled", otherwise this action is skipped.
...
are not installed.  Runs as the last operational script to ensure a new server
has all available security updates applied.  Runs if the 
<tt>rightscale/security_update</tt> input is set to "Enabled", otherwise 
this action is skipped.

== USAGE:

=== Update a server with latest security patches

1. Prior to launching a sever set the advanced input "rightscale/security_update"
   to "Enabled"
2. When the server is operational all current security updates are applied.
3. Apply security updates using:
  
    "rightscale/security_update"
   to bring the server to the latest patch level.

=== Enable security updates on a running server

1. On a running a sever set the advanced input "rightscale/security_update"
   to "Enabled"
2. Setup the package manger for security updates using:

    "rightscale::rightscale::setup_security_updates"
