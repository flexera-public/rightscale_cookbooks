...
This cookbook contains recipes that generally applicable to all types of application
servers.

Initialized App resource and its actions cover a default set of operations that are
required for setting up typical application servers.
...
1. Add the default recipe to tag your server as an application server. The 'app' 
   tag is used by servers (like database servers) to identify their clients.
2. Add desired <tt>app::do_*</tt> recipes to the ServerTemplate for your application server.
3. Develop corresponding providers in your application server's cookbook.
4. Define required and recommended attributes in your application server's
...
For now we have <tt>app_php</tt>, <tt>app_tomcat</tt> and <tt>app_passenger</tt> cookbooks.
Each of them contain the implementation of corresponding app__* Lightweight Providers 
