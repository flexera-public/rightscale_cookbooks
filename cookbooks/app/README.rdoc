...
This cookbook contains recipes, required to connect your instance to RightScale Load balancer ServerTemplate
...
This cookbook provides abstract <tt>app</tt> resource. Which will act as "interface" for corresponding Light Weight app_* providers

For now we have app_php, app_tomcat and app_passenger cookbooks. Each of them contain implementation of corresponding app__* Light Weight Provider
which can be called using this resource.

Provider persisted in this cookbook will act as a cap if there will be no another app_ Light Weight Provider implementation. To avoid unexpected fails.
