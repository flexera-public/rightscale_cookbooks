...
This is an abstract cookbook that defines the recipes and interface to application provider
cookbooks.   It is not a standalone cookbook and must be used in conjunction with an
application provider cookbook (i.e. app_php, app_tomcat, app_passenger or a user created application provider)
...
1. Add the app_*::default recipe prior to the app::default recipe to define the provider.
...
3. Add desired app::do_* recipes to your application server ServerTemplate
4. Develop corresponding providers in your application server cookbook.
5. Define required and recommended attributes in your application server cookbook attributes file.
...
This includes app_php, app_tomcat and app_passenger cookbooks. Each of them contain implementation of corresponding app__* LightWeight Provider
...
All actions declared in this resource define the interface for typical application server operations.
