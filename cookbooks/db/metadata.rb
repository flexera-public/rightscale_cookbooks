...
recipe "db::default", "Selects and installs database client. " +
  "It also sets up the provider and version for 'db' resource."

recipe "db::install_server", "Installs and sets up the packages " +
  "that are required for database servers. Adds the database:active=true tag " +
  "to your server, which identifies it as a database server. " +
  "The tag is used by application servers to identify active databases."

recipe "db::setup_monitoring", "Installs the collectd plugin for database " +
  "monitoring support, which is required to enable monitoring and alerting " +
  "functionality for your servers."

# == Common Database Recipes
#
recipe "db::do_primary_backup",
  :description => "Creates a primary backup of the database using persistent " +
    "storage in the current cloud. Backup type depends on cloud and " +
    "hypervisor type. For clouds with volume snapshots support available, " +
    "volume backup will be used only if hypervisor is different than KVM. " +
    "For clouds without volume snapshots support and for KVM based instances " +
    "backups are uploaded to ROS container.",
  :thread => 'db_backup'

recipe "db::do_primary_restore", "Restores the database from the most " +
  "recently completed primary backup available in persistent storage " +
...
recipe "db::do_secondary_backup",
  :description => "Creates a backup of the database and uploads it to a " +
    "secondary cloud storage location, which can be used to migrate your " +
    "database to a different cloud. For example, you can save a secondary " +
    "backup to an Amazon S3 bucket or a Rackspace Cloud Files container.",
  :thread => 'db_backup'
...
#
recipe "db::do_appservers_allow", "Allows connections from all application " +
  "servers in the deployment that are tagged with appserver:active=true tag. " +
  "This script should be run on a database server so that " +
  "it will accept connections from related application servers."

recipe "db::do_appservers_deny", "Denies connections from all application " +
  "servers in the deployment that are tagged with appserver:active=true tag. " +
  "This script can be run on a database server to deny connections from all " +
  "application servers in the deployment."

recipe "db::request_appserver_allow", "Sends a request to allow connections " +
  "from the caller's private IP address to all database servers " +
  "in the deployment that are tagged with the database:active=true tag. " +
  "This should be run on an application server " +
  "before attempting a database connection."

recipe "db::request_appserver_deny", "Sends a request to deny connections " +
  "from the caller's private IP address to all database servers " +
  "in the deployment that are tagged with the database:active=true tag. " +
...
#
recipe "db::do_init_and_become_master", "Initializes the database " +
  "and tags it as the master database server. " +
  "Sets DNS. Starts a fresh backup from this master."

recipe "db::do_primary_restore_and_become_master", "Restores the database " +
  "and tags it as the master database server. " +
  "Sets DNS. Starts a fresh backup from this master."

recipe "db::do_secondary_restore_and_become_master", "Restores the database " +
  "from a secondary backup location and tags it as the master database " +
  "server. Sets DNS. Starts a fresh backup from this master."

recipe "db::do_primary_init_slave", "Initializes the slave server " +
  "from the primary backup location. " +
  "Authentication information provided by inputs is ignored for slave servers."

recipe "db::do_secondary_init_slave", "Initializes the slave server " +
  "from the secondary backup location. " +
  "Authentication information provided by inputs is ignored for slave servers."
...
recipe "db::request_master_allow", "Sends a request to the master database " +
  "server tagged with rs_dbrepl:master_instance_uuid=<master_instance_uuid> " +
...
recipe "db::request_master_deny", "Sends a request to the master database " +
  "server tagged with rs_dbrepl:master_instance_uuid=<master_instance_uuid> " +
...
recipe "db::do_delete_volumes_and_terminate_server", "Deletes any currently " +
  "attached volumes from the instance and then terminates the machine."
...
attribute "db/dns/master/fqdn",
  ...
  :description => "The fully qualified domain name for the master database " +
    "server. Example: db-master.example.com",
  :required => "required",
  :recipes => ["db::default", "db::install_server"]

attribute "db/dns/master/id",
  :display_name => "Database Master DNS Record ID",
  :description => "The unique identifier that is associated with " +
    "the DNS A record of the master database server. " +
    "The unique identifier is assigned by the DNS provider when you create " +
    "a dynamic DNS A record. This ID is used to update " +
    "the associated A record with the private IP address of the master " +
    "server when this recipe is run. If you are using DNS Made Easy " +
    "as your DNS provider, a 7-digit number is used (e.g., 4403234). "+
    "Example:4403234",
  :required => "required",
  :recipes => [
...
attribute "db/dns/slave/fqdn",
  ...
  :description => "The fully qualified domain name for a slave database " +
    "server. Example: db-slave.example.com",
  :required => "optional",
...
attribute "db/dns/slave/id",
  ...
  :description => "The unique identifier that is associated with " +
    "the DNS A record of a slave server. " +
    "The unique identifier is assigned by the DNS provider " +
...
attribute "db/admin/user",
  ...
  :description => "The username of the database user with 'admin' " +
    "privileges. Example: cred:DBADMIN_USER.",
  :required => "required",
  :recipes => [
    "db::install_server",
...
attribute "db/admin/password",
  ...
  :description => "The password of the database user with 'admin' " +
    "privileges. Example: cred:DBADMIN_PASSWORD.",
  :required => "required",
  :recipes => [
    "db::install_server",
    "db::setup_privileges_admin",
...
attribute "db/replication/user",
  ...
  :description => "The username of the database user that has 'replication' " +
    "privileges. Example: cred:DBREPLICATION_USER.",
  :required => "required",
  :recipes => [
...
attribute "db/replication/password",
  ...
  :description => "The password of the database user that has 'replication' " +
    "privileges. Example: cred:DBREPLICATION_PASSWORD.",
  :required => "required",
  :recipes => [
    "db::setup_replication_privileges",
    "db::do_primary_restore_and_become_master",
...
attribute "db/application/user",
  ...
  :description => "The username of the database user that has 'user' " +
    "privileges. Example: cred:DBAPPLICATION_USER.",
  :required => "required",
  :recipes => [
    "db::default",
...
attribute "db/application/password",
  ...
  :description => "The password of the database user that has 'user' " +
    "privileges. Example: cred:DBAPPLICATION_PASSWORD.",
  :required => "required",
  :recipes => [
    "db::default",
    "db::setup_privileges_application",
...
attribute "db/init_slave_at_boot",
  ...
  :description => "Set to 'True' to have the instance initialize " +
    "the database server as a slave on boot. " +
    "Set to 'False' if there is no master database server running. " +
    "Example: false",
  :default => "false",
  :choice => ["true", "false"],
...
attribute "db/provider_type",
  ...
  :description => "Database provider type to use on client side. " +
    "This must be a string containing the provider cookbook name " +
    "and (optionally) the version of the database. " +
...
attribute "db/backup/lineage",
  ...
  :description => "The prefix that will be used to name/locate the backup" +
    ...
    "the setup process creates one. Example: text:prod_db_lineage",
  :required => "required",
  :recipes => [
    "db::do_primary_init_slave",
    "db::do_secondary_init_slave",
...
attribute "db/backup/lineage_override",
  ...
  :description => "If defined, this will override the input defined for " +
    "'Backup Lineage' (db/backup/lineage) so that you can restore " +
    "the database from another backup that has as a different lineage name. " +
    ...
    "Be sure to remove the 'Backup Lineage Override' " +
    "input after the new master is operational. Example: text:new_db_lineage",
  :required => "optional",
  :recipes => [
    "db::do_init_slave_at_boot",
    "db::do_primary_restore_and_become_master",
...
attribute "db/backup/restore_version_check",
  ...
  :description => "A variable for allowing the restoration of a database " +
    "from a backup that was performed on a different version " +
    "of the DB software. Make sure you fully understand the implications " +
    "of cross-version restoration.  Set to 'False' to skip version checking. " +
    "Example: False",
  :required => "optional",
  :choice => ["true", "false"],
  :default => "true",
...
attribute "db/backup/primary/master/cron/hour",
  ...
  :description => "Defines the hour of the day when the primary backup " +
    "will be taken of the master database. " +
    "Backups of the master are taken daily. " +
    ...
    "then backups will occur once per day at the " +
...
attribute "db/backup/primary/master/cron/minute",
  ...
  :description => "Defines the minute of the hour when the backup " +
    "will be taken of the master database. " +
    "Backups of the master are taken daily. " +
...
attribute "db/dump/storage_account_provider",
  ...
  :description => "Location where the dump file will be saved. " +
...
attribute "db/dump/storage_account_id",
  ...
  :description => "In order to write the dump file to the specified cloud " +
...
attribute "db/dump/container",
  ...
  :description => "The cloud storage location where the dump file " +
    "will be saved to or restored from. For Amazon S3, use the bucket name. " +
...
attribute "db/dump/database_name",
  ...
  :description => "Enter the name of the database name/schema " +
    "to create/restore a dump from/for. Example: mydbschema",
  :required => "required",
  :recipes => [
    "db::do_dump_import",
    "db::do_dump_export",
