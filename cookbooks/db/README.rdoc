...
* Place <tt>db::install_server</tt> recipe into your runlist to install
  database client and server. This recipe should be run after the db provider
  recipe "setup_server_<version>" which sets up provider, version and server
  packages to be installed for db resource. For example:

    db_mysql::setup_server_5_5
    db::install_server

  This will setup and install MySQL 5.5 client and server.

* Place <tt>db::default</tt> recipe into your runlist to install database client 
  and set up provider and version for db resource. When using a 
  RightScale ServerTemplate, this will also automatically add the common
...
* The database provider to use is defined by the <tt>node[:db][:provider]</tt> attribute. You
