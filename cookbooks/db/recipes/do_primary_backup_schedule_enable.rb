...
if node[:db][:this_is_master]
  if master_hour.empty? || master_minute.empty?
    log "  Using default setting to skip scheduling primary backup cron job on master db server"
  else
    log "  Setting up Master primary backup cron job to run at hour: #{master_hour} and minute #{master_minute}"
...
# Setup Slave schedule
else
  log "  Setting up Slave primary backup cron job to run at hour: #{slave_hour} and minute #{slave_minute}"
