...
download source code from GIT repositories.

== REQUIREMENTS:

* Requires repo Light Weight resource cookbook https://github.com/rightscale/rightscale_cookbooks/tree/master/cookbooks/repo
...
* Once setup, use the recipes in the 'repo' cookbook to download your code repository.
  See the repo/README.rdoc for usage details.

* Select 'repo_git' option in repo/default/provider input.

* The repo/url, and repo/branch inputs are required to use this provider cookbook.

* To use a private git repositories the repo/ssh_key input must be set.
  We recommend using a 'key' or 'cred' type variable from RightScale dashboard.
...
===== Actions:
<tt>:pull</tt>
Standard respository pull. Your source code repository will be pulled from remote url to specified destination.

<tt>:capistrano_pull</tt>
Perform standard repository pull and then capistrano deployment style will be applied.
