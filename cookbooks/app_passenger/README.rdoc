...
== DETAILS:
=== General
<tt>app_passenger</tt> cookbook will help you to install and setup Apache + Phusion passenger application server, wich can be able to connect, to MySQL or Postgres database server.
also:
...
* <tt>:code_update</tt> - Perform project source code update/download using user selected "repo" LWRP. Setup logrotation config.

* <tt>:setup_db_connection</tt> - Perform project config/database.yml database file configuration
