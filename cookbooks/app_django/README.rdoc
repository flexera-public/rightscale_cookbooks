...
* <tt>Bundler python packages support</tt>
...
1 - For apps with bundler support, add the "requirements.txt" file which includes the packages, and
"run_custom_python_commands" for migration commands

2 - For apps without bundler support, gives package name at input level while server launch, and
"run_custom_python_commands" for migration commands
...
* <tt>settings.py.erb</tt> - move the existing django settings.py if found in docroot to settings_default.py and Configure new settings.py
