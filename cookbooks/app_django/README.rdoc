...
* Tested on RedHat 6.2 RightImage
...
Python version 2.6.6 (2012-06-18) (x86_64-linux)
Pip version 1.1
...
* <tt>Automatic logrotate file configuration for Django logs</tt>

For more info see: http://support.rightscale.com/18-Release_Notes/ServerTemplates_and_RightImages/2012-01-05#Apache-Django_App_Server_-_Beta

ServerTemplate built on this cookbook can be combined in two modes:

1 - For apps with bundler support, add the "install_required_app_python_modules" recipe, and
"run_custom_python_commands" for migration commands

2 - For apps without bundler support, add "install_custom_python_packages" recipe, and
"run_custom_python_commands" for migration commands
...
* <tt>node[:app_django][:apache][:user]</tt> - Apache user. Required for changing owner of created project dirs.
...
* <tt>settings.erb</tt> - move the existing django settings.py to settings_default.py and Configure new settings.py 
  for project database connection config file used in the <tt>app::setup_db_connection</tt> recipe
  and defined in <tt>:setup_db_connection</tt> action of the <tt>app_django</tt> LWRP.
* <tt>logrotate_django.erb</tt> - Configuration for logrotate used in the <tt>app::setup_vhost</tt> recipe
  and defined in <tt>:setup_vhost</tt> action of the <tt>app_django</tt> LWRP.
