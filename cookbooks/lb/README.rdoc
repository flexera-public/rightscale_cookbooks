...
This cookbook is intended to provide an interface for general load balancer actions and parameters. 
...
There is a generic lb_client resource provider included in this cookbook.  This provider is intended to be used on application servers that need to attach to a load balancer.  The lb_client provider only supports the :attach_request and :detach_request actions. 

The :attach_request and :detach_request actions will send a remote recipe to all load balancers in the deployment with the "loadbalancer:#{pool_name}=lb" machine tag.  The :attach_request action will request them to run the "lb::handle_attach" recipe.  The :detach_request will request them to run the "lb::handle_dettach" recipe.
...
For more examples see the lb::do_attach_request and lb::do_detach_request recipes.
