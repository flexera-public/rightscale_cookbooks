...
recipe "lb::install_server", "Installs the load balancer " +
  "and adds the loadbalancer:<pool_name>=lb tags to your server, " +
  "which identifies it as a load balancer for a given listener pool. " +
  "This tag is used by application servers to request connection/disconnection."
...
recipe "lb::do_attach_all", "Registers all running application servers " +
...
recipe "lb::do_attach_request", "Sends request to all load balancers " +
  "to attach the current server to the listener pool using given parameters " +
...
recipe "lb::do_detach_request", "Sends request to all load balancers " +
  "to detach the current server from the listener pool " +
  "(For lb_haproxy - lb/pools input value will be used. " +
  "For ELB/CLB - lb/service/* input values will be used). " +
...
recipe "lb::setup_advanced_configuration",
  "Recipe for advanced load balancer configuration setup."

attribute "lb/pools",
  :display_name => "Load Balance Pools",
  :description => "Comma-separated list of URIs or FQDNs " +
    "for which the load balancer will create server pools to answer " +
    "website requests. Last entry will be the default backend and will answer " +
    "for all URIs and FQDNs not listed here. A single entry of any name, " +
    "e.g. 'default', 'www.mysite.com' or '/appserver', will mimic basic " +
    "behavior of one load balancer with one pool of application servers. " +
    "This will be used for naming server pool backends. Application servers can " +
    "provide any numbers of URIs or FQDNs to join corresponding server " +
    "pool backends. Example: www.mysite.com, api.mysite.com, /serverid, default",
...
attribute "lb/service/provider",
  ...
  :description => "Specify the load balance provider to use: " +
...
attribute "lb/service/region",
  ...
  :description => "For Rackspace's Cloud Load Balancing service region, " +
...
attribute "lb/service/lb_name",
  ...
  :description => "Name of the Cloud Load Balancer " +
    "or Elastic Load Balancer device. Example: mylb",
  :required => "optional",
  :recipes => [
    "lb::default",
    "lb::do_attach_request",
...
attribute "lb/service/account_id",
  ...
  :description => "The account name that is required for access to " +
  "specified cloud load balancer. " +
  "For Rackspace's CLB service, use your Rackspace username. " +
  "(e.g., cred: RACKSPACE_USERNAME). " +
...
attribute "lb/service/account_secret",
  ...
  :description => "The account secret that is required for access to " +
