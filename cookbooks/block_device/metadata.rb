...
attribute "block_device/devices/default/backup/secondary/cred/user",
  ...
  :description => "Secondary cloud authentication credentials. For Rackspace Cloud Files, use your Rackspace login username (e.g., cred:RACKSPACE_USERNAME). For OpenStack Swift the format is: <tenant-id>:<user-name>. For Amazon S3, use your Amazon key ID (e.g., cred:AWS_ACCESS_KEY_ID). Example: cred:CLOUD_ACCOUNT_USERNAME",
  :required => "recommended",
  :default => "",
...
devices.sort.each_with_index.map do |device, index|
  ...
  attribute "block_device/devices/#{device}/device_type",
    ...
    :description => "The type of the device - SATA or SSD. This attribute is supported only on Rackspace Open Cloud. Example: SATA",
    :type => "string",
    :required => "optional",
    :choice => ["SATA", "SSD"],
    :default => "SATA",
