...
recipe "block_device::do_primary_backup",
  :description => "Creates a primary backup in the local cloud " +
    "where the server is currently running.",
  :thread => 'block_backup'
...
attribute "block_device/devices_to_use",
  ...
  :description => "The block device(s) to operate on. " +
    "Can be a comma-separated list of device names or '*' " +
    "to indicate all devices. Example: device1",
...
attribute "block_device/devices/default/backup/primary/cred/user",
  ...
  :description => "Primary cloud authentication credentials. " +
    "For Rackspace Cloud Files, use your Rackspace login username " +
      "(e.g., cred:RACKSPACE_USERNAME). For OpenStack Swift the format is: " +
      "<tenant-id>:<user-name>. For clouds that do not require primary " +
      "credentials (e.g., Amazon), set to 'ignore'. " +
...
attribute "block_device/devices/default/backup/primary/cred/secret",
  ...
  :description => "Primary cloud authentication credentials. " +
    ...
    "(e.g., cred:RACKSPACE_AUTH_KEY). For clouds that do not require primary " +
    "credentials (e.g., Amazon), set to 'ignore'. " +
...
attribute "block_device/devices/default/backup/primary/cloud",
  ...
  :description => "The primary backup storage cloud " +
...
attribute "block_device/devices/default/backup/primary/endpoint",
  ...
  :description => "The endpoint URL for the primary backup storage cloud. " +
    "This is used to override the default endpoint " +
    "or for generic storage clouds such as Swift. " +
...
attribute "block_device/devices/default/backup/secondary/cred/user",
  ...
  :description => "Secondary cloud authentication credentials. " +
    "For Rackspace Cloud Files, use your Rackspace login username " +
    "(e.g., cred:RACKSPACE_USERNAME). For OpenStack Swift the format is: " +
    "<tenant-id>:<user-name>. For Amazon S3, use your Amazon key ID " +
    "(e.g., cred:AWS_ACCESS_KEY_ID). Example: cred:CLOUD_ACCOUNT_USERNAME",
  :required => "recommended",
  :default => "",
  :recipes => [
...
attribute "block_device/devices/default/backup/secondary/cloud",
  ...
  :description => "The secondary backup storage cloud. " +
...
attribute "block_device/devices/default/backup/secondary/endpoint",
  ...
  :description => "The endpoint URL for the secondary backup storage cloud. " +
    "This is used to override the default endpoint " +
    "or for generic storage clouds such as Swift. " +
    "Example: http://endpoint_ip:5000/v2.0/tokens",
  :required => "optional",
...
attribute "block_device/devices/default/backup/rackspace_snet",
  ...
  :description => "When 'true', Rackspace internal private networking " +
    "(preferred) is used for communications between servers " +
    "and Rackspace Cloud Files. Ignored for all other clouds. Example: true",
  :type => "string",
  :required => "optional",
  :choice => ["true", "false"],
  :default => "true",
...
devices.sort.each_with_index.map do |device, index|
  ...
  attribute "block_device/devices/#{device}/volume_size",
    ...
    :description => "Defines the total size of the LVM volume stripe set " +
      "(in GB). For example, if the stripe_count is '3' " +
      "and you specify '3' for this input, it will create an LVM volume " +
      "stripe that contains 3 volumes that are each 1 GB in size. " +
      "If an uneven ratio is defined, volume sizes will be rounded up " +
      "to the nearest whole integer. Ignored on clouds that do not support " +
      "volumes (e.g., Rackspace). Example: 10",
    :required => device != 'device2' ? 'recommended' : 'optional',
    :default => "10",
    :recipes => ["block_device::setup_block_device", "block_device::default"]
  ...
  attribute "block_device/devices/#{device}/backup/lineage",
    ...
    :description => "The name associated with your primary and " +
      "secondary database backups. It's used to associate them with " +
      "your database environment for maintenance, restore, " +
      "and replication purposes. Backup snapshots will automatically " +
      "be tagged with this value (e.g. rs_backup:lineage=mysqlbackup). " +
      "Backups are identified by their lineage name. " +
  ...
  attribute "block_device/devices/#{device}/nickname",
    ...
    :description => "The name displayed in the dashboard for volumes " +
      "and to uniquely identify LVM volume groups. Example: data_storage1",
    :required => device != 'device2' ? 'recommended' : 'optional',
  ...
  attribute "block_device/devices/#{device}/backup/lineage_override",
    ...
    :description => "If defined, this will override the input " +
      "defined for 'Backup Lineage' " +
  ...
  attribute "block_device/devices/#{device}/backup/timestamp_override",
    ...
    :description => "Another optional variable to restore from a specific " +
  ...
  attribute "block_device/devices/#{device}/backup/primary/cron/minute",
    ...
    :description => "Defines the minute of the hour when " +
      "the backup will be taken. Use a value of 1-59, or set to 'Ignore' and " +
      "a random minute will be calculated. Example: 10",
    :required => "optional",
    :default => "",
  ...
  attribute "block_device/devices/#{device}/backup/primary/keep/max_snapshots",
    ...
    :description => "The maximum number of primary backups to keep " +
      "in addition to those being rotated.  Example: 60",
    :required => "optional",
    :default => "60",
    :recipes => ["block_device::default"]
  ...
  attribute "block_device/devices/#{device}/backup/primary/keep/daily",
    ...
    :description => "The number of daily primary backups to keep " +
      "(i.e., rotation size). Example: 14",
    :required => "optional",
    :default => "14",
  ...
  attribute "block_device/devices/#{device}/backup/primary/keep/weekly",
    ...
    :description => "The number of weekly primary backups to keep " +
      "(i.e., rotation size). Example: 6",
    :required => "optional",
    :default => "6",
    :recipes => backup_recipes + ["block_device::default"]
  ...
  attribute "block_device/devices/#{device}/backup/primary/keep/monthly",
    ...
    :description => "The number of monthly primary backups to keep " +
      "(i.e., rotation size). Example: 12",
    :required => "optional",
    :default => "12",
    :recipes => backup_recipes + ["block_device::default"]
  ...
  attribute "block_device/devices/#{device}/backup/primary/keep/yearly",
    ...
    :description => "The number of yearly primary backups to keep " +
      "(i.e., rotation size). Example: 2",
    :required => "optional",
    :default => "2",
    :recipes => backup_recipes + ["block_device::default"]
  ...
  attribute "block_device/devices/#{device}/mount_point",
    ...
    :description => "The directory of where to mount the block device " +
      "(e.g., /mnt/storage). Example: /mnt/storage",
    :type => "string",
    :required => device != 'device2' ? 'recommended' : 'optional',
  ...
  attribute "block_device/devices/#{device}/vg_data_percentage",
    ...
    :description => "The percentage of the total Volume Group extents (LVM) " +
      "that is used for data. (e.g. 50 percent - 1/2 used for data " +
      "and remainder used for overhead and snapshots, " +
      "100 percent - all space is allocated for data (therefore snapshots " +
      "can not be taken) WARNING: If the space used for data storage is " +
      "too large, LVM snapshots cannot be performed. " +
  ...
  attribute "block_device/devices/#{device}/iops",
    ...
    :description => "The input/output operations per second (IOPS) " +
      "that the volume can support. IOPS is currently only supported on " +
      "Amazon EC2. Example: 500",
    :type => "string",
    :required => "optional",
    :recipes => ["block_device::setup_block_device", "block_device::default"]
  ...
  attribute "block_device/devices/#{device}/device_type",
    ...
    :description => "The type of the device - SATA or SSD. " +
      "This attribute is supported only on Rackspace Open Cloud. " +
      "Example: SATA",
    :type => "string",
    :required => "optional",
    :choice => ["SATA", "SSD"],
...
attribute "block_device/terminate_safety",
  ...
  "of the block_device::do_teminate_server recipe. " +
  "The recipe will only run if the input variable is overridden " +
...
attribute "block_device/force_safety",
  ...
  :description => "Prevents the accidental running " +
  "of the block_device::do_force_reset recipe. " +
  "The recipe will only run if the input variable is overridden " +
  "and set to \"off\". Example: text:off",
  :type => "string",
  :required => "recommended",
