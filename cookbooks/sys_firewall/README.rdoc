...
only outbound connections are allowed. <tt>sys_firewall::default</tt>
...
<tt>sys_firewall::default</tt> recipe increase the value for the
...
packets on high-throughput systems. The IPTABLES_MODULES_UNLOAD config option is
set to "no" to prevent drop of calculated values during iptables reconfiguration
 on Centos/Rhel images.
