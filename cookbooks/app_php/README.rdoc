...
== DETAILS:
=== General
<tt>app_php</tt> cookbook will help you to install and setup Apache + Zend Engine PHP application server, which can connect to a MySQL or Postgres database server.
...
====== Platform dependent
* <tt>node[:php][:module_dependencies]</tt> - additional packages required for php server install
...
<tt>app_php</tt> Light Weight provider is defined in providers/default.rb file, it contains source for the following actions:
