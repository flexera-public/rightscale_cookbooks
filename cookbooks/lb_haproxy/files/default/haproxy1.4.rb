...
def usage(code=0)
  ...
  out << "\tThis collectd exec plugin is indended to collect the statistic\n"
...
stats = []
	...
	stat = {}
		...
		# Grab values
			...
			if ! value[id].nil?
                            stat[column[id].to_sym] = value[id]
                          end
			end
		end
	...
	stat[:status] = case stat[:status].to_s.downcase
		when /no check/ then "NOCHECK"
		when /up \d+\/\d+/ then "GOINGDOWN"
		when /down \d+\/\d+/ then "GOINGUP"
		else stat[:status].to_s.upcase
	end

	#Append the hash to a list, so each line is processed
        stats << stat unless stat[:svname].nil?
	end
stats
end
...
## Prints totals from backend instance then
## Prints data for individual instances
####
def outputstats(now)
